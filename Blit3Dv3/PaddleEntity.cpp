...
PaddleEntity * MakePaddle(float xpos, float ypos, Sprite *sprite, float scale)
{
	...
	boxShape.SetAsBox((paddleEntity->halfWidth) * scale, (paddleEntity->halfHeight) * scale);
	...
	paddleFixtureDef.filter.categoryBits = CMASK_PADDLE;
	paddleFixtureDef.filter.maskBits = CMASK_BALL | CMASK_POWERUP;
